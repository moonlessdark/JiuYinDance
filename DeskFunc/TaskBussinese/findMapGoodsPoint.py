...
class FindMapGoodsPointList:
    ...
    def __init__(self):
        self._get_point_list: list = get_map_goods_point_list()
        self._map_pic = GetConfig().get_map_pic()  # 地图
        self.windows = FindWindowsImageTemplate()
        self.windows_cap = WindowsCapture()
        self._status_config = GetConfig().get_goods_opt_status()
        self.find_point = FindPoint()
    ...
    def search_goods_point(self, x: str, y: str, hwnd: int):
        ...
        pic_w = self.windows_cap.capture(hwnd).pic_width
    ...
    def check_person_move_status(self, hwnd: int) -> bool:
        """
        检测人物角色行走状态。
        ...
        for i in range(2):
            ...
            pic_str = self.find_point.find_button_sort(pic_content)
            if _last_ocr_str == "":
                _last_ocr_str = pic_str
            else:
                if _last_ocr_str == pic_str:
                    # 2次识别坐标都一致，说明站着不动了
                    return True
