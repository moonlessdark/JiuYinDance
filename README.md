...
2、授业时最多只支持2个号在同一个场景同时授业，因为免费版大漠不持支绑定窗口，所以只能按了一轮候再切换另一个窗口按按钮。3个号的时候时间来不及。 建议真的要3开授业时，最好分开到2个场景。   
3、请以管理员权限运行，不然大漠插件无法正常运行。  
4、此脚本暂不支持win7，在win10系统运行良好，win11的话不知道行不行。pyqt5对win7的兼容性我真的处理不来~  
