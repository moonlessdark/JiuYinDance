...
### 关于成语搜索  
* 由于OCR识别精度较差，会出现识别错误或者漏识别的问题，请自己手动补充完善一下。  
* 本脚本一共收录了接近5万个成语，应该能够覆盖大部分的成语了。    
* 感谢: https://github.com/mapull/chinese-dictionary/tree/main/idiom  
...
新增:  
1、9点自动开卡  
注意:只开2次，从8点59分55秒开始检测  


2024年8月21日  
优化:  
1、成语搜索改为使用 11*9的表格展示，方便修补识别错误/漏掉的文字。  
2、本地成语文件更新为接近5万个成语，应该能覆盖大部分的情况了。  


2024年8月19日  
新增:  
1、沙漠地图的机关锁(成语填空)辅助功能,将列举出所有可能的成语(但不保证能列举出所有的成语,毕竟成语列表是从互联网获取的)。  
注意:由于免费的OCR识别精度不太行，经常有错误或者漏掉文字的情况,请在获取文字后手动修复补充一下。  
优化:  
1、打开软件时自动勾选已经启动的游戏窗口。  
